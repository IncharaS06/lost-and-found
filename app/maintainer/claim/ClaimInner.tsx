...
export default function MaintainerClaimInner({ claimId }: { claimId: string }) {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      try {
        ...
        const userRole = userSnap.exists() ? (userSnap.data() as any)?.role : "";

        // ✅ allow admin/teacher also if you want (optional)
        if (!userSnap.exists() || !["maintainer", "admin", "teacher"].includes(String(userRole))) {
        ...
        <div className="rounded-2xl border border-green-800/25 bg-black/40 p-5 shadow-xl shadow-green-900/10">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            ...
            <div className="rounded-xl border border-green-800/20 bg-black/35 p-3">
              ...
              <p className="text-white text-sm mt-1">
                {claim?.officeHours || "10:00 AM – 4:00 PM"}
              </p>
            </div>
          </div>
          ...
          {(claim?.assignedMaintainerName || claim?.collectionPoint) && (
            <div className="mt-3 rounded-xl border border-green-800/20 bg-green-900/10 p-3">
              ...
              <p className="text-white text-sm mt-1">
                {claim?.assignedMaintainerName || "Maintainer Office"}
              </p>
              <p className="text-white/80 text-sm">
                📍 {claim?.collectionPoint || "Central Office / Security Desk"}
              </p>
            </div>
          )}
        </div>

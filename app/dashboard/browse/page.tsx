...
type Item = {
  ...
  // ✅ OLD format (data URL) - may exist in old docs
  imageData?: string | null;

  // ✅ NEW format (pure base64 + mime)
...
/** ✅ pick the best image source (supports all 3 formats) */
function getItemImageSrc(i: Item): string | null {
  // 1) old base64 data URL
  if (i.imageData && String(i.imageData).startsWith("data:")) {
    return i.imageData;
  }

  // 2) new pure base64 + mime
  if (i.imageBase64 && i.imageBase64.length > 0) {
    const mime = i.imageMime || "image/webp";
    return `data:${mime};base64,${i.imageBase64}`;
  }

  // 3) legacy storage/public URL
  if (i.imageUrl && i.imageUrl.length > 0) {
    return i.imageUrl;
  }
...
export default function BrowsePage() {
  ...
  useEffect(() => {
    ...
    const unsubLost = onSnapshot(
      ...
      (err) => {
        console.error("lost_items snapshot error:", err);
        setLoading(false);
      }
    );
    ...
    const unsubFound = onSnapshot(
      ...
      (snap) => {
        ...
        // ✅ debug: check first doc fields
        const first: any = snap.docs[0]?.data();
        if (first) {
          console.log("[found_items] sample keys:", Object.keys(first));
          console.log("[found_items] has imageBase64?", !!first.imageBase64, "has imageData?", !!first.imageData);
        }
      ...
      (err) => {
        console.error("found_items snapshot error:", err);
        setLoading(false);
      }
    );

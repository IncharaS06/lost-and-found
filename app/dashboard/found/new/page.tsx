...
/**
 * ✅ Compress + convert to WebP + return PURE base64 (no data:image/... prefix)
 */
async function fileToCompressedBase64(
    ...
    if (!base64) throw new Error("Failed to generate base64 image");
...
export default function ReportFoundPage() {
    ...
    const getPhotoPrompt = () => {
        if (category === "ID Card") return "Front side (show name and photo clearly)";
        if (category === "Wallet") return "Show wallet open with cards visible";
        if (category === "Phone") return "Front and back (show model, color, case)";
        if (category === "Keys") return "All keys on keychain clearly visible";
        if (category === "Laptop") return "Open laptop showing screen and stickers";
        if (category === "Bag") return "Whole bag from multiple angles";
        if (category === "Notebook") return "Cover and first page with writing";
        if (category === "Charger") return "Plug type and cable length visible";
        if (category === "Water Bottle") return "Show brand logo and distinctive marks";
    ...
    const submit = async () => {
        ...
        if (!loc) return alert("Please enter found location.");
        if (!foundDate) return alert("Please pick the found date.");

        // Validate date is within current year
        const selectedYear = new Date(foundDate).getFullYear();
        ...
        setSubmitting(true);
        try {
            // ✅ Store PURE base64 + store mime separately
            let imageBase64 = "";
            let imageMime = "";

            if (file) {
                // First try
                const r1 = await fileToCompressedBase64(file, 900, 0.7);
                imageBase64 = r1.base64;
                imageMime = r1.mime;

                // Approx size check (base64 length ~ bytes * 4/3)
                // Keep it below ~950k chars to be safe in Firestore doc size
                if (imageBase64.length > 850_000) {
                    const r2 = await fileToCompressedBase64(file, 700, 0.6);
            ...
            await addDoc(collection(db, "found_items"), {
                ...
                // ✅ Base64 storage (PURE)
                imageBase64, // PURE base64 string
                imageMime, // e.g. image/webp

                status: "open",

                // who found it
                foundBy: uid,
                finderEmail: user?.email || auth.currentUser?.email || "",
            ...
            alert(
                `Found item posted!\nAssigned to: ${assignee.assignedMaintainerName}\nSubmit at: ${assignee.collectionPoint} (${assignee.officeHours})`
            );
            ...
            <div className="max-w-xl mx-auto relative">
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <p className="text-white/80 text-sm font-medium">Report Found Item</p>
                    </div>
                </div>
                    ...
                    >
                        <Field label="Item Title" icon={<FileText className="h-4 w-4 text-green-400" />}>
                            <input
                                ...
                                placeholder="e.g., ID Card / Wallet / Keys"
                            />
                        </Field>
                        ...
                        <Field label="Found Location" icon={<MapPin className="h-4 w-4 text-green-400" />}>
                            <input
                                value={foundLocation}
                        ...
                        {/* NOTE: keeping your layout, just fixing minor DOM semantics */}
                        <div>
                            <label className="text-white/70 text-xs">Found Date</label>
                            <div className="mt-1 flex items-center gap-2 rounded-xl border border-green-800/30 bg-black/40 px-3 py-3 transition focus-within:border-green-500/60 focus-within:ring-2 focus-within:ring-green-500/20">
                            ...
                            <div className="mt-1 text-xs text-white/50">Only {currentYear} dates allowed</div>
                        </div>
                            ...
                            {!preview && (
                                <div className="mt-3 rounded-xl border border-green-800/20 bg-black/30 p-4">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-white/40 text-xs">
                                                Helps owner verify faster (auto-compressed to WebP).
                                            </p>
                                        </div>
                                    </div>
                                    <div className="bg-green-900/10 border border-green-800/20 rounded-lg p-3 mt-2">
                                        ...
                                        <p className="text-white/70 text-xs">{getPhotoPrompt()}</p>
                                    </div>
                                </div>
                            )}
                            ...
                            {preview && (
                                <div className="mt-3">
                                    <div className="mb-2 flex items-center justify-between">
                                        ...
                                        <div className="text-white/50 text-xs">{getPhotoPrompt()}</div>
                                    </div>
...
/**
 * ✅ HOW TO DISPLAY LATER (example):
 * const src = item.imageBase64 ? `data:${item.imageMime};base64,${item.imageBase64}` : "/placeholder.png";
 */

...
// ✅ Student email parser & validator: vvce<yy><branch><4-digit-roll>@vvce.ac.in
...
export default function RegisterPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(niceFirebaseError(err?.code) || err?.message || "Registration failed");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-gray-950 via-black to-green-950/30 flex items-center justify-center px-4 py-10">
      ...
      >
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <div
            className={cn(
              "transition-all duration-700 delay-[360ms]",
              mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"
            )}
          >
        ...
        <p
          className={cn(
            "mt-5 text-xs text-white/50 text-center transition-all duration-700 delay-500",
            mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"
          )}
        >
          {role === "teacher"
